...
- 🏗️ [Arsitektur dan Struktur Proyek](#️-arsitektur-dan-struktur-proyek)
- 📋 [Prasyarat](#-prasyarat)
...
4. ☁️ Data transaksi disimpan ke Firestore pada ruang milik user tersebut.
...
- 🛡️ Account linking agar akun guest dapat diamankan dengan Google atau email
...
- ✏️ Edit nominal alokasi dari halaman History maupun Savings.
- ↩️ Tarik seluruh alokasi dengan konfirmasi dan pemulihan saldo otomatis.
- 🧾 Lihat ringkasan serta riwayat aktivitas alokasi.
- 🗃️ Pisahkan target aktif dan target selesai.
...
- 🏷️ Filter berdasarkan kategori transaksi.
...
- 🕶️ Privacy Mode untuk menyamarkan saldo pada Dashboard.
- 👤 Nama pengguna dan tipe profil keuangan.
- ☁️ Status sinkronisasi Firestore dengan waktu pembaruan terakhir.
...
| ☁️ Database | Cloud Firestore | Menyimpan transaksi dan target secara real-time |
...
> ℹ️ Project ini menggunakan **Riverpod**, bukan package Provider atau BLoC.
...
├── macos/                           # 🖥️ Konfigurasi desktop macOS
...
│       ├── settings/                # ⚙️ Pengaturan dan manajemen data
...
- ☁️ **Data** menerjemahkan data Firestore menjadi entity yang dipahami domain.
...
- 🛠️ FlutterFire CLI jika ingin menghasilkan ulang konfigurasi Firebase.
...
- 🏷️ Package name aplikasi adalah `com.example.money_tracker`.
...
- 🏷️ Pastikan bundle ID Firebase dan Xcode sama.
...
- ☁️ Deploy Hosting agar file `.well-known/assetlinks.json` dapat diakses publik.
...
- ☁️ Hosting mengambil file dari `build/web`.
...
- 🗂️ Transaksi berada di `users/{uid}/transactions`.
...
- 🕶️ Privacy Mode hanya menyamarkan nominal pada Dashboard dan tidak mengubah
